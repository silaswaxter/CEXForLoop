...
def generate_header_file(file_path, max_nttp_count, linear_expansion_length, linear_expansion_count):
    ...
    with open(file_path, "w") as f:
        ...
        for t in range(max_nttp_count):
            ...
            if t == 0:
                f.write("    // This implementation differs from the others in that it adapts 0 user\n")
                f.write("    // NTTPs to the 1 user NTTPs interface. Adhering to the pattern of the other\n")
                f.write("    // implementations results in a O(n) complexity for instantiation depth\n")
                f.write("    // versus required template depth rather than the desired O(log(n))\n")
                f.write("    // complexity. I (Silas Waxter) tried debugging why. I diffed the 0 user\n")
                f.write("    // NTTPs with same pattern implementation versus the 1 user NTTPs\n")
                f.write("    // implementation, and looked at each difference trying to see why it would\n")
                f.write("    // cause a different behavior. I even wrote some mocks to try to make the 0\n")
                f.write("    // user NTTPs mimic the 1 user NTTPs at the spots in which they different,\n")
                f.write("    // but this was to no avail. I ended up spending more hours than I cared, so\n")
                f.write("    // I decided to write this adapter/hider which works. The extra couple\n")
                f.write("    // templates and extra function call at **compile time** is not that\n")
                f.write("    // expensive so its okay.\n")
